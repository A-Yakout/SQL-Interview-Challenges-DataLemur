...
**Difficulty:** 🟡 Medium
**Link:** [View Problem on DataLemur](https://datalemur.com/questions/uncategorized-calls-percentage)
...
1. Used Conditional Aggregation that counts only "n/a" and empty cells .
...
```sql
SELECT 
  ROUND(COUNT(CASE WHEN call_category is NULL OR call_category = 'n/a' THEN 1 END)*100.0 /
...
SELECT 
  ROUND(
    100.0 * AVG(CASE WHEN call_category IS NULL OR call_category = 'n/a' THEN 1 ELSE 0 END),
    1
